...
app.register(async function sanitize(instance) {
  instance.addHook('onRequest', async (req) => {
    const s = (obj) => { if (typeof obj !== 'object' || !obj) return; for (let k in obj) { if (typeof obj[k] === 'string') obj[k]=obj[k].replace(/<[^>]*>/g,'').replace(/['"]/g,''); else s(obj[k]); } };
    s(req.body); s(req.query); s(req.params);
  });
});
...
app.get('/fallback', async (req, reply) => reply.type('text/html').send('<html><body style=font-family:sans-serif;padding:2em><h1>InternOps API</h1><a href=/docs>Swagger</a></body></html>'));
